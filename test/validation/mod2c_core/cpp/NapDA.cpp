...
#endif /*BBCORE*/
  register_mech(_mechanism, nrn_alloc,nrn_cur, NULL, nrn_state, nrn_init, _create_global_variables, _destroy_global_variables, hoc_nrnpointerindex, 4);
...
static void _update_global_variables(NrnThread *_nt, Memb_list *_ml) {
 ...
 #define _slist2 static_cast<_global_variables_t*>(_ml->instance)->_slist2
...
_thread = _ml->_thread;
  ...
  if (!_newtonspace1) {
    ...
    if (_nt->compute_gpu) {
      ...
      double* _d_pd = cnrn_target_copyin(_thread[_dith1]._pval, 2*2*_cntml_padded);
